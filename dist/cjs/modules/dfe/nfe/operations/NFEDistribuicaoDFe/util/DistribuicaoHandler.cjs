...
class DistribuicaoHandler {
    ...
    deCompressDFeXML(loteDistDFeInt, metodo, xmlConsulta) {
        try {
            ...
            const files = [];
            xml2js_1.default.parseString(loteDistDFeInt, (err, result) => {
                if (err) {
    ...
    decodeDocZip(docZip) {
        ...
        let decompressedData = pako_1.default.inflate(bytes);
    ...
    parseXml(xmlString) {
        let parsedResult;
        xml2js_1.default.parseString(xmlString, (err, result) => {
            if (err) {
...
exports.default = DistribuicaoHandler;
