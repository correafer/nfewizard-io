...
class NFEGerarDanfe {
    constructor(props) {
        ...
        this.productTableCurrentY = 0;
        this.productTableHeaderHeight = 15; // defaultItemHeight in _buildProdutos
        ...
        this.enviada = false;
        this.documento = new ValidaCPFCNPJ_1.default();
    ...
    _buildGuia() {
        ...
        this.doc.fontSize(6).text(`EMISSÃO: ${(0, date_fns_1.format)(new Date(this.ide.dhEmi), 'dd-MM-yyyy')} -  DEST. / REM.: ${this.dest?.xNome || ''}  -  VALOR TOTAL: R$ ${parseFloat(String(this.total.ICMSTot.vNF)).toFixed(2)}`, 10, 33.5, {
            characterSpacing: 0.5
        });
        /** RIGHT */
        ...
        const _buildIdentificacaoNFe = () => {
            ...
            if (Number(this.ide.tpAmb) === 2 || this.enviada) {
                ...
                if (this.protNFe?.infProt.dhRecbto) {
                    dhRecbto = (0, date_fns_1.format)(new Date(this.protNFe?.infProt.dhRecbto), 'dd/MM/yyyy HH:mm:ss');
                }
    ...
    _buildDestinatario() {
        ...
        const _buildDestPessoa = () => {
            ...
            this.doc.fontSize(8).text((0, date_fns_1.format)(new Date(this.ide.dhEmi), 'dd-MM-yyyy'), left + 497, topDestinatario + 135, {
                characterSpacing: 1,
            });
        };
        const _buildDestLogradouro = () => {
            ...
            this.doc.fontSize(8).text((0, date_fns_1.format)(new Date(), 'dd-MM-yyyy'), left + 497, topDestinatario + 158, {
                characterSpacing: 1,
            });
        };
        const _buildDestEndereco = () => {
            ...
            this.doc.fontSize(8).text((0, date_fns_1.format)(new Date(), 'HH:mm'), left + 497, topDestinatario + 181, {
                characterSpacing: 1,
            });
        };
        ...
        try {
            ...
            if (this.logoUrl) {
                if (this.logoUrl.startsWith('http://') || this.logoUrl.startsWith('https://')) {
                    try {
                        const response = await axios_1.default.get(this.logoUrl, { responseType: 'arraybuffer' });
                    ...
                    if (fs_1.default.existsSync(this.logoUrl)) {
                        try {
                            this.logoBuffer = fs_1.default.readFileSync(this.logoUrl);
                        }
                        catch (error) {
